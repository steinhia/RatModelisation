...
class SliderGrp(object):
    def __init__(self,title,buttonList,checkBoxList,nameList,pointOnCurveList):
        ...
        cmds.rowColumnLayout(numberOfColumns=4,columnWidth=[(1,1),(2,300),(4,50),(5,50)])
        ...
        # CheckBox
        cmds.setParent('..')
        cmds.rowColumnLayout( numberOfColumns=2,columnWidth=[(1, 150),(2,200),(3,200)])
...
def functionButton(sliderList,crvInfos,i,sliderName,nameAfter,min,max,min2,max2,value,step,function,valueReset=0,getFunction=0,getFunctionArgs=[],setOneFunction=setOneRot,keepPosition=True,keepPosture=True):
    ...
    slider=SliderOffset(sliderName,action,min,max,value,step,sliderList)
    action2 = FunctionAction(getValue,crvInfos,setRot,args=[slider,getFunction,getFunctionArgs,min,max,crvInfos],mvt=True,keepPosture=keepPosture,keepPosition=keepPosition)
...
def createWindows(nameList,pointOnCurveList):
    ...
    buttonList.append(functionButton(sliderList,crvInfos,4,names[4],namesAfter[4],-4,4,-90,90,0,0.00001,setFcts[0],getFunction=fcts[4]))
    buttonList.append(functionButton(sliderList,crvInfos,5,names[5],namesAfter[5],-3,4,-80,80,0,0.00001,setFcts[1],getFunction=fcts[5],keepPosture=False,keepPosition=False))

    # dorsales GD HB -> dorsales HB = dorsales + lombaires
    buttonList.append(functionButton(sliderList,crvInfos,6,names[6],namesAfter[6],-3,3,-80,80,0,0.00001,setFcts[2],getFunction=fcts[6]))
    buttonList.append(functionButton(sliderList,crvInfos,7,names[7],namesAfter[7],-3,3,-80,80,0,0.00001,setFcts[3],getFunction=fcts[7],keepPosture=False,keepPosition=False))

    #lombaires
    buttonList.append(functionButton(sliderList,crvInfos,8,names[8],namesAfter[8],-6,6,-80,80,0,0.00001,setFcts[4],getFunction=fcts[8]))
    buttonList.append(functionButton(sliderList,crvInfos,9,names[9],namesAfter[9],-6,6,-80,80,0,0.00001,setFcts[5],getFunction=fcts[9],keepPosture=False,keepPosition=False))

    # compression
    buttonList.append(functionButton(sliderList,crvInfos,10,names[10],namesAfter[10],-10,100,1,85,0,0.00001,setFcts[6],crvInfos,getFunction=fcts[10],getFunctionArgs=crvInfos,keepPosition=True))

    # position
    buttonList.append(postureButton(sliderList,crvInfos,11,names[11],namesAfter[11],-5,3,0.00001,["curve1"],crvInfos[1][0],functionSet=setX,keepPosition=False,keepCurveLength=False,keepPosture=False))
    buttonList.append(postureButton(sliderList,crvInfos,12,names[12],namesAfter[12],4,10,0.00001,["curve1"],crvInfos[1][1],functionSet=setY,keepPosition=False,keepCurveLength=False,keepPosture=False))
    buttonList.append(postureButton(sliderList,crvInfos,13,names[13],namesAfter[13],-5,5,0.00001,["curve1"],crvInfos[1][2],functionSet=setZ,keepPosition=False,keepCurveLength=False,keepPosture=False))
    # scaling
    buttonList.append(postureButton(sliderList,crvInfos,14,names[14],namesAfter[14],3,15,0.00001,["curve1"],valueReset=getCurveLength(),valueSetTo=1,functionSet=setFcts[10],keepCurveLength=False,keepPosition=True,args=[],name2="set to 1"))
    # posture generale
    buttonList.append(postureButton(sliderList,crvInfos,3,names[3],namesAfter[3],-90,90,0.00001,["curve1"],0,crvInfos[1],functionSet=setFcts[11],args=[],keepPosture=False,keepPosition=True,keepCurveLength=False))
    buttonList.append(postureButton(sliderList,crvInfos,15,names[15],namesAfter[15],-90,90,0.00001,["curve1"],0,crvInfos[1],functionSet=setFcts[12],args=[],keepPosture=False,keepPosition=True,keepCurveLength=False))
    ...
    sliderGrp1=SliderGrp("Modelisation de la colonne du rat",buttonList,checkBoxList,nameList,pointOnCurveList)
